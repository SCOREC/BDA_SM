...
def get_result():
    return forward_request(target_url=result_cache_host + "/api/get_result")

@RCResource.route('/get_status', methods=['GET'])
@initialized(True)
@require_access(['train', 'infer'])
def get_status():
    return forward_request(target_url=result_cache_host + "/api/get_status")

@RCResource.route('/get_eta', methods=['GET'])
@initialized(True)
@require_access(['train', 'infer'])
def get_eta():
    return forward_request(target_url=result_cache_host + "/api/get_eta")